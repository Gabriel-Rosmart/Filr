            ...
            <div class="stat">
                ...
                <div class="stat-value text-xl whitespace-normal">
                    <a :href="'/user/permit/download/justification?uuid=' + permit.uuid" target="_blank" class="dark:hover:text-cyan-400 hover:underline">
                        {{ permit.file ? permit.file : t('permits.noJustif') }}
                    </a>
                </div>
            </div>
            <div class="stat">
                <div class="stat-title">{{ t('permits.attachOrChange') }}</div>
                <form @submit.prevent="submit" enctype="multipart/form-data">
                    <FormFileInput id="file" name="file"
                            v-on:change="upFile" />
                    <button type="submit" class="btn btn-primary mt-2 ml-5" @click="submit">{{ permit.file ? t('permits.change') : t('permits.attach') }}</button>
                </form>
            </div>
        </div>
    </div>
